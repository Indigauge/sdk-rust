...
use crate::text_edit::virtual_keyboard::{VirtualKey, VirtualKeyboard, VirtualKeyboardPlugin, VirtualKeyboardPos};

use bevy::app::{App, Plugin, Update};
use bevy::input::ButtonState;
use bevy::input::keyboard::{Key, KeyboardInput};
use bevy::prelude::{
  Alpha, ButtonInput, Changed, Commands, Component, Deref, DerefMut, Entity, EntityEvent, GlobalTransform,
  IntoScheduleConfigs, KeyCode, Message, MessageReader, MessageWriter, MouseButton, Query, Res, ResMut, Resource,
  States, Text, Time, Timer, TimerMode, Touches, With, Without, in_state,
};
...
macro_rules! plugin_systems {
  ( ) => {
    (listen_changing_focus, focus_text_box, listen_keyboard_input, blink_cursor, display_placeholder).chain()
  };
}
  ...
  T: States,
{
  fn build(&self, app: &mut App) {
    app
      .add_plugins(VirtualKeyboardPlugin::new(self.states.clone()))
...
pub fn listen_changing_focus(
  ...
  virtual_keyboard_interaction: Query<
    &Interaction,
    (Changed<Interaction>, With<VirtualKeyboard>, Without<VirtualKey>, Without<TextEditable>),
  >,
