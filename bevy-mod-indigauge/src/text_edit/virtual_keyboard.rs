use crate::text_edit::auto_timer::{ActionOnFinish, AutoTimer, AutoTimerFinished, AutoTimerPlugin};
use crate::text_edit::{TextEditConfig, TextFocusChanged};
use bevy::app::{App, Plugin, Startup};
use bevy::ecs::relationship::RelatedSpawnerCommands;
use bevy::input::ButtonState;
use bevy::input::keyboard::{Key, KeyboardInput};
use bevy::prelude::{
  AlignContent, AlignSelf, BorderColor, ChildOf, Color, Commands, Component, Deref, DerefMut, Entity, EntityEvent,
  Gamepad, GamepadButton, Handle, Image, ImageNode, Interaction, IntoScheduleConfigs, JustifyItems, KeyCode, Luminance,
  Message, MessageReader, MessageWriter, Node, On, Out, Pointer, Press, Query, Release, Res, ResMut, Resource, Single,
  States, Text, TextColor, TextFont, Timer, TimerMode, Update, Visibility, Window, With, ZIndex, in_state, on_message,
};
  ...
  T: States,
{
  fn build(&self, app: &mut App) {
    app
      .add_plugins(AutoTimerPlugin::new(self.states.clone()))
  ...
  fn from(keys: Vec<Vec<((&str, &str), KeyCode, Option<(Key, Key)>, f32)>>) -> Self {
    ...
    for row in keys {
      ...
      for (label, key_code, logical_key, size) in row {
        let logical_key = logical_key.unwrap_or((Key::Character(label.0.into()), Key::Character(label.1.into())));
  ...
  fn default() -> Self {
    Self::from(vec![
      vec![
        ...
        (("Backspace", "BACKSPACE"), KeyCode::Backspace, Some((Key::Backspace, Key::Backspace)), 2.),
      ],
      vec![
      ...
      vec![
        (("Shift", "SHIFT"), KeyCode::ShiftLeft, Some((Key::Shift, Key::Shift)), 1.5),
      ...
      vec![
        ...
        (("<=", "<="), KeyCode::ArrowLeft, Some((Key::ArrowLeft, Key::ArrowLeft)), 1.),
        (("=>", "=>"), KeyCode::ArrowRight, Some((Key::ArrowRight, Key::ArrowRight)), 1.),
      ],
    ])
  }
}
...
fn on_pointer_press(
  ...
  on_press(trigger.entity, &mut keys, &mut event, windows, &mut virtual_keyboard, &mut text, config);
}
...
fn on_press(
  ...
  if let Ok(window) = windows.single()
    && let Ok((key, mut timer)) = keys.get_mut(target)
  {
...
fn on_repeat(
  ...
  if let Ok(window) = windows.single()
    && let Ok((key, mut timer)) = keys.get_mut(trigger.entity)
  {
    let logical_key = if virtual_keyboard.show_alt {
      key.logical_key.1.clone()
    } else {
