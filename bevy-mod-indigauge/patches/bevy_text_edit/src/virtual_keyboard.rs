...
use bevy::prelude::{
    ...
    JustifyItems, KeyCode, Luminance, Message, MessageReader, MessageWriter, Node, On, Out, Pointer, Press, Query,
    Release, Res, ResMut, Resource, Single, States, Text, TextColor, TextFont, Timer, TimerMode, Update, Visibility,
    Window, With, ZIndex,
};
...
fn on_repeat(
    ...
    {
        let logical_key = if virtual_keyboard.show_alt { key.logical_key.1.clone() } else { key.logical_key.0.clone() };
        event.write(KeyboardInput {
            key_code: key.key_code,
            logical_key,
            state: ButtonState::Pressed,
